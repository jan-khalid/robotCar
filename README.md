...
<strong style="color: #40E0D0; text-decoration: underline;">Function 1: Going to a particular coordinate without obstacles</strong> requires QTI sensors as its main sensors. There are four QTI sensors installed at the bottom of the vehicle, as shown in Figure 12. The front two QTI sensors are combined to detect the straight black line on the ground, ensuring the vehicle moves on a straight line. The QTI sensors installed under the motors are used to count the number of black lines passed in relation to a specific coordinate of the grid.
